 ...
 */
class InfoBoxBuilder extends AbstractBuilder
{
     ...
     * @param \Ivory\GoogleMapBundle\Model\Base\CoordinateBuilder $coordinateBuilder The coordinate builder.
     *
     * @return \Ivory\GoogleMapBundle\Model\Overlays\InfoBoxBuilder The builder.
     */
    ...
    {
        $infoBox = new $this->class();

        if ($this->prefixJavascriptVariable !== null) {
            $infoBox->setPrefixJavascriptVariable($this->prefixJavascriptVariable);
        }
        ...
        if (!empty($this->pixelOffset)) {
            ...
            $infoBox->setPixelOffset($pixelOffset);
        }

        if ($this->open !== null) {
            $infoBox->setOpen($this->open);
        }

        if ($this->openEvent !== null) {
            $infoBox->setOpenEvent($this->openEvent);
        }

        if ($this->autoOpen !== null) {
            $infoBox->setAutoOpen($this->autoOpen);
        }

        if ($this->autoClose !== null) {
            $infoBox->setAutoClose($this->autoClose);
        }

        if (!empty($this->options)) {
            $infoBox->setOptions($this->options);
        }
